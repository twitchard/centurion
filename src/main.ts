...
logConnection(
  `Multiplayer build ${__MULTIPLAYER_BUILD_ID__}: MQTT + BitTorrent + Nostr (app id ${CENTURION_TRANSPORT_APP_ID}). Hard-refresh both devices if this build id looks stale.`,
)
navigate(pathnameToAppRoute(window.location.pathname), false)
tryRestorePersistedSession()
autoJoinFromUrl()
